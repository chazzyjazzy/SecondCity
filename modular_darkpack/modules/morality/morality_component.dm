...
/datum/component/morality/proc/adjust_morality(datum/source, type, limit)
	...
	*/
	...
	if(!GLOB.canon_event)
		return
	/* TODO: [Rebase] reimplement... 'is_special_character'?
	if(is_special_character(kindred))
		return
	*/
	/* TODO: [Rebase] reimplement frenzy
	if(kindred.in_frenzy)
		return
	*/
	// Don't get another path hit if cooldown is up
	if(TIMER_COOLDOWN_RUNNING(kindred.morality_path, COOLDOWN_PATH_HIT))
		return
	...
	switch(kindred.morality_path.alignment)
		if(MORALITY_HUMANITY)
			switch(SIGN(type))
				if(PATH_SCORE_UP)
					path_rating += 1
					SEND_SOUND(kindred, sound('modular_darkpack/modules/deprecated/sounds/humanity_gain.ogg', 0, 0, 75))
					to_chat(kindred, span_userdanger("<b>HUMANITY INCREASED!</b>"))
				if(PATH_SCORE_DOWN)
					if(path_rating-1 < limit)
						return
					path_rating -= 1
					SEND_SOUND(kindred, sound('modular_darkpack/modules/deprecated/sounds/humanity_loss.ogg', 0, 0, 75))
